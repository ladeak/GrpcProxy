...
public class ProxyServiceMethodProviderContext
{
    ...
    //public void AddClientStreamingMethod<TService, TRequest, TResponse>(Method<TRequest, TResponse> method, IList<object> metadata, ClientStreamingServerMethod<TService, TRequest, TResponse> invoker)
    //    where TRequest : class
    //    where TResponse : class
    //    where TService : class
    //{
    //    var callHandler = _serverCallHandlerFactory.CreateClientStreaming(method, invoker);
    //    AddMethod(method, RoutePatternFactory.Parse(method.FullName), metadata, callHandler.HandleCallAsync);
    //}
