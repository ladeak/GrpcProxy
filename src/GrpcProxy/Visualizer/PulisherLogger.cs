...
internal static class PulisherLogger
{
    ...
    public static void RequestMessage(ILogger logger, ProxyMessage request) => _requestMessage(logger, request.Timestamp, request.Endpoint, request.MethodType, request.Path, request.Message.ToString() ?? string.Empty, request.ProxyCallId, null);

    public static void ResponseMessage(ILogger logger, ProxyMessage response) => _responseMessage(logger, response.Timestamp, response.Endpoint, response.MethodType, response.Message.ToString() ?? string.Empty, (int?)response.StatusCode ?? 0, response.ProxyCallId, null);
}